...
- **Scales to bigger worlds.** A **World** control (Cozy / Balanced / Grand) trades cell size for canvas detail. Under the hood, an **active-region tracker** bounds the heavy simulation passes to just the part of the world that's actually doing something, so a sprawling, mostly-quiet world stays smooth.
